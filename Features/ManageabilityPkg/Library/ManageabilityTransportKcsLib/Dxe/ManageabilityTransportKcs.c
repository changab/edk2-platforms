...
EFI_GUID * SupportedManageanilityProtocol [] = {
  &gManageabilityProtocolIpmiGuid,
  &gManageabilityProtocolMctpGuid
};

UINT8  NumberOfSupportedProtocol = (sizeof(SupportedManageanilityProtocol)/sizeof(EFI_GUID *));

MANAGEABILITY_TRANSPORT_KCS_HARDWARE_INFO mKcsHarwareInfo;
...
/**
  ...
  @param [in]  HardwareInfo             The hardware information
                                        assigend to KCS transport interface.
  ...
  )
{
  ...
  if (HardwareInfo.Kcs == NULL) {
    DEBUG((DEBUG_INFO, "%a: Hardware information is not provided, use dafult settings.\n", __FUNCTION__));
    mKcsHarwareInfo.MemoryMap                    = MANAGEABILITY_TRANSPORT_KCS_IO_MAP_IO;
    mKcsHarwareInfo.IoBaseAddress.IoAddress16    = PcdGet16(PcdIpmiKcsBaseAddress);
    mKcsHarwareInfo.IoDataInAddress.IoAddress16  = mKcsHarwareInfo.IoBaseAddress.IoAddress16 + IPMI_KCS_DATA_IN_REGISTER_OFFSET;
    mKcsHarwareInfo.IoDataOutAddress.IoAddress16 = mKcsHarwareInfo.IoBaseAddress.IoAddress16 + IPMI_KCS_DATA_OUT_REGISTER_OFFSET;
    mKcsHarwareInfo.IoCommandAddress.IoAddress16 = mKcsHarwareInfo.IoBaseAddress.IoAddress16 + IPMI_KCS_COMMAND_REGISTER_OFFSET;
    mKcsHarwareInfo.IoStatusAddress.IoAddress16  = mKcsHarwareInfo.IoBaseAddress.IoAddress16 + IPMI_KCS_STATUS_REGISTER_OFFSET;
  } else {
    mKcsHarwareInfo.MemoryMap        = ((MANAGEABILITY_TRANSPORT_KCS_HARDWARE_INFO *)HardwareInfo.Kcs)->MemoryMap;
    mKcsHarwareInfo.IoBaseAddress    = ((MANAGEABILITY_TRANSPORT_KCS_HARDWARE_INFO *)HardwareInfo.Kcs)->IoBaseAddress;
    mKcsHarwareInfo.IoDataInAddress  = ((MANAGEABILITY_TRANSPORT_KCS_HARDWARE_INFO *)HardwareInfo.Kcs)->IoDataInAddress;
    mKcsHarwareInfo.IoDataOutAddress = ((MANAGEABILITY_TRANSPORT_KCS_HARDWARE_INFO *)HardwareInfo.Kcs)->IoDataOutAddress;
    mKcsHarwareInfo.IoCommandAddress = ((MANAGEABILITY_TRANSPORT_KCS_HARDWARE_INFO *)HardwareInfo.Kcs)->IoCommandAddress;
    mKcsHarwareInfo.IoStatusAddress  = ((MANAGEABILITY_TRANSPORT_KCS_HARDWARE_INFO *)HardwareInfo.Kcs)->IoStatusAddress;
  }
  ...
  if (mKcsHarwareInfo.MemoryMap) {
    DEBUG((DEBUG_INFO, "Memory Map I/O\n", __FUNCTION__));
    DEBUG((DEBUG_INFO, "Base Memory Address : 0x%08x\n", mKcsHarwareInfo.IoBaseAddress.IoAddress32));
    DEBUG((DEBUG_INFO, "Data in Address     : 0x%08x\n", mKcsHarwareInfo.IoDataInAddress.IoAddress32));
    DEBUG((DEBUG_INFO, "Data out Address    : 0x%08x\n", mKcsHarwareInfo.IoDataOutAddress.IoAddress32));
    DEBUG((DEBUG_INFO, "Command Address     : 0x%08x\n", mKcsHarwareInfo.IoCommandAddress.IoAddress32));
    DEBUG((DEBUG_INFO, "Status Address      : 0x%08x\n", mKcsHarwareInfo.IoStatusAddress.IoAddress32));
  } else {
    DEBUG((DEBUG_INFO, "I/O Map I/O\n"));
    DEBUG((DEBUG_INFO, "Base I/O port    : 0x%04x\n", mKcsHarwareInfo.IoBaseAddress.IoAddress16));
    DEBUG((DEBUG_INFO, "Data in I/O port : 0x%04x\n", mKcsHarwareInfo.IoDataInAddress.IoAddress16));
    DEBUG((DEBUG_INFO, "Data out I/O port: 0x%04x\n", mKcsHarwareInfo.IoDataOutAddress.IoAddress16));
    DEBUG((DEBUG_INFO, "Command I/O port : 0x%04x\n", mKcsHarwareInfo.IoCommandAddress.IoAddress16));
    DEBUG((DEBUG_INFO, "Status I/O port  : 0x%04x\n", mKcsHarwareInfo.IoStatusAddress.IoAddress16));
  }
  return EFI_SUCCESS;
}
  ...
  )
{
  ...
  Status = HelperManageabilityCheckSupportedSpec (
             ...
             SupportedManageanilityProtocol,
  ...
  KcsTransportToken->Token.Transport->TransportVersion                    = MANAGEABILITY_TRASNPORT_TOKEN_VERSION;
  ...
  KcsTransportToken->Token.Transport->Fucntion.Version1_0 = AllocateZeroPool (sizeof(MANAGEABILITY_TRANSPORT_FUNCTION_V1_0));
  if (KcsTransportToken->Token.Transport->Fucntion.Version1_0 == NULL) {
    DEBUG((DEBUG_ERROR, "%a: Fail to allocate memory for MANAGEABILITY_TRANSPORT_FUNCTION_V1_0\n", __FUNCTION__));
    return EFI_OUT_OF_RESOURCES;
  }
  KcsTransportToken->Token.Transport->Fucntion.Version1_0->TransportInit            = KcsTransportInit;
  KcsTransportToken->Token.Transport->Fucntion.Version1_0->TransportReset           = KcsTransportReset;
  KcsTransportToken->Token.Transport->Fucntion.Version1_0->TransportStatus          = KcsTransportStatus;
  KcsTransportToken->Token.Transport->Fucntion.Version1_0->TransportTransmitReceive = KcsTransportTransmitReceive;
  ...
  )
{
  ...
  if (KcsTransportToken != NULL) {
    FreePool (KcsTransportToken->Token.Transport->Fucntion.Version1_0);
